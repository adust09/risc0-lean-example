...
def processAttestation (state : BeaconState) (attestation : Attestation) : STFResult BeaconState :=
  ...
  if data.target.epoch != previousEpoch && data.target.epoch != currentEpoch then
    ...
    if attestationEpoch != data.target.epoch then
      ...
      let mut state := state
      let proposerRewardNumerator : UInt64 := 0
      ...
      for bitIdx in [:numBits] do
        ...
        let bitPos := bitIdx % 8
        let isSet := if h : byteIdx < attestation.aggregationBits.size then
          (attestation.aggregationBits.get ⟨byteIdx, h⟩).toNat >>> bitPos &&& 1 == 1
        else false
        if isSet then
          ...
          if h : i < participation.size then
            ...
            if newFlags != flags then
              let newParticipation := participation.set ⟨i, h⟩ newFlags
