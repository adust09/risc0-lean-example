...
def processEffectiveBalanceUpdates (state : BeaconState) : BeaconState :=
  ...
  for i in [:validators.size] do
    if h : i < validators.size then
      let validator := validators[i]
      if h2 : i < state.balances.size then
        ...
        if balance + downwardThreshold < validator.effectiveBalance ||
          ...
          validators := validators.set ⟨i, h⟩ newVal
  { state with validators := validators }
